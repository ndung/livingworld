    ...
    <s:layout-component name="contents">
        <div class="panel">
            <div class="panel-body">
                <s:form class="form-horizontal" beanclass="id.co.icg.ie.web.user.UserManagementActionBean">
                ...
                <div class="table-primary">
                    <div class="dataTables_table_wrapper">
                        <d:table class="table table-striped table-bordered dataTable text-center" name="${actionBean.list}" id="obj" requestURI="" defaultsort="1" partialList="true" size="${actionBean.list.fullListSize}" pagesize="${actionBean.list.objectsPerPage}">
                            <d:column title="User Name"     property="id"           sortable="true" style="width: 10%; text-align: center"/>    
                            <d:column title="Full Name"     property="fullName"     sortable="true" style="width: 15%; text-align: center"/>
                            <d:column title="Email"         property="email"        sortable="false" style="width: 13%; text-align: center"/>
                            <d:column title="Phone"         property="phone"        sortable="false" style="width: 10%; text-align: center"/>
                            <d:column title="Role"          property="appRole.name" sortable="false" style="width: 11%; text-align: center"/>
                            <d:column title="Status"        property="status"       sortable="true" style="width: 5%; text-align: center" decorator="id.co.icg.ie.displaytag.StatusFormatDecorator"/>
                            <d:column title="Action" style="width: 10%; text-align: center">
                                <s:link beanclass="id.co.icg.ie.web.user.EditUserActionBean" class="btn btn-sm">
                                    <s:param name="appUser" value="${obj}"/>
                                    Edit
                                </s:link>
                                <c:if test="${obj.status == 1}">
                                    <s:link beanclass="id.co.icg.ie.web.user.UserManagementActionBean" event="deactivate" class="btn btn-sm confirm">
                                        <s:param name="appUser" value="${obj}"/>
                                        Deactivate
                                    </s:link>
                                </c:if>
                                <c:if test="${obj.status == 0}">
                                    <s:link beanclass="id.co.icg.ie.web.user.UserManagementActionBean" event="activate" class="btn btn-sm confirm">
                                        <s:param name="appUser" value="${obj}"/>
                                        Activate
                                    </s:link>
                                </c:if>
