...
@Service
public class EcashApiServiceImpl extends RetrofitClient<EcashApi> implements EcashApiService {
    ...
    @Override
    public ValidateResponse validate(String phoneNumber, String token) throws Exception{
        Response<ValidateResponse> response = service.validateAccount(phoneNumber, token).execute();
        return response.body();
    }

    @Override
    public TicketResponse getTicket(String mobileNumber, String token) throws Exception{
